...
def build_pdf_from_records(records: List[Dict], show_titles: bool = False, batch_id: str = "", presentation_title: str = "") -> bytes:
    """
    Creates a PDF presentation from the records.
    """
    if not PDF_GEN_SUPPORT:
        st.error("PDF generation not available - reportlab not installed")
        return b""
    ...
    # Add title page if presentation title is provided
    if presentation_title:
        story.append(Paragraph(presentation_title, title_style))
        story.append(Paragraph("Generated by U2P - Upload to Present", subtitle_style))
        story.append(Paragraph(f"Batch ID: {batch_id} • {datetime.now().strftime('%B %d, %Y')}", subtitle_style))
        story.append(Paragraph(f"Total Files: {len(records)}", subtitle_style))
        story.append(Spacer(1, 30))
        
        # Add file list if not too many
        if len(records) <= 10:
            story.append(Paragraph("Files Included:", subtitle_style))
            for rec in records[:8]:  # Limit to 8 files
                is_camera = rec.get("is_camera", False)
                camera_display = " (Camera)" if is_camera else ""
                custom_title = rec.get("title", "")
                
                if custom_title and custom_title != rec["name"]:
                    file_text = f"• {custom_title} - {rec['name']}{camera_display}"
                else:
                    file_text = f"• {rec['name']}{camera_display}"
                
                story.append(Paragraph(file_text, styles['Normal']))
            
            if len(records) > 8:
                story.append(Paragraph(f"... and {len(records) - 8} more files", styles['Normal']))
    ...
    for rec in sorted_records:
        ...
        if is_image(fname):
            try:
                ...
                # Convert image to bytes for reportlab
                img_bytes = io.BytesIO()
                img.save(img_bytes, format='PNG')
                img_bytes.seek(0)
                ...
                story.append(RLImage(img_bytes, width=img_width, height=img_height))
                story.append(Spacer(1, 20))
                
            except Exception as e:
                st.warning(f"⚠️ Could not process image '{fname}' for PDF: {str(e)}")
                others.append(rec)
            ...
            try:
                ...
                for page_num in range(len(pdf_document)):
                    ...
                    img_width = 6 * inch
                    img_height = img_width * pix.height / pix.width
                ...
                st.warning(f"Could not process PDF '{fname}' for PDF: {str(e)}")
                others.append(rec)
        else:
            others.append(rec)
    ...
    if others:
        story.append(Paragraph("Attached Files", slide_title_style))
        for rec in others:
            story.append(Paragraph(f"• {rec['name']}", styles['Normal']))
        story.append(Spacer(1, 20))
    
    # Add footer
    if batch_id:
        story.append(Paragraph(f"Batch: {batch_id}", subtitle_style))
...
# Step 3: Download
    ...
    with col2:
        ...
        # Generate PDF
        pdf_bytes = build_pdf_from_records(st.session_state.records, st.session_state.show_titles, st.session_state.batch_id, st.session_state.presentation_title)
        # Generate filename based on presentation title
        if st.session_state.presentation_title:
            # Clean the title for filename use
            clean_title = "".join(c for c in st.session_state.presentation_title if c.isalnum() or c in (' ', '-', '_')).rstrip()
            clean_title = clean_title.replace(' ', '_')[:30]  # Limit length
            filename = f"{clean_title}_{st.session_state.batch_id}.pdf"
        else:
            filename = f"u2p_{st.session_state.batch_id}.pdf"
